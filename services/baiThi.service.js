...
const getListExam = async ({username}) => {
    const sql = `select BaiThi.id, DATE_FORMAT(BaiThi.NgayTao,'%d/%m/%Y') as NgayTao , BaiThi.TrangThai, DeThi.Ten  
    from BaiThi, DeThi 
...
const createEmptyExam = async ({idExam}, {username}) => {
    ...
    for(let i = 0 ; i < parts.length ; i++){
        const part = await deThiService.getPart({idExam: idExam, idPart: parts[i]});
        for(let j = 0 ; j < part.length ; j++){
            for(let k = 0 ; k < part[j].cauHoi.length ; k++){
                await DBaiThiService.create({id_BaiThi: idBaiThi, id_CauHoi: part[j].cauHoi[k].id})
            }
        }
    } 
