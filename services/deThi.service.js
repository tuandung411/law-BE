...
const createEmptyExam = async (id_DeThi) => {
    for(let i = 0 ; i < subParts.length ; i++){
        ...
        const id_subPart = await subPartService.create(id_Part, id_DeThi, Ten, '');
        if(id_Part === 'L' || id_Part === 'R'){
            const {tu, den, diem} = subParts[i];
            for(let j = tu ; j <= den ; j ++){
                await cauHoiService.create({id_Part: subParts[i].id_Part, id_SubPart: id_subPart, NoiDung: '', So: j, Diem: diem, DapAn: ''})
            }
        }
        else if(id_Part === 'W'){
            const {diem} = subParts[i];
            await cauHoiService.create({id_Part: subParts[i].id_Part, id_SubPart: id_subPart, NoiDung: '', So: 1, Diem: diem, DapAn: ''})
        } 
        else if(id_Part === 'S'){
            const {diem} = subParts[i];
            await cauHoiService.create({id_Part: subParts[i].id_Part, id_SubPart: id_subPart, NoiDung: '', So: 1, Diem: diem, DapAn: 'Speaking'})
        }
    }     
}
