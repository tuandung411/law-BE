...
const create = async ({name, username, password, birthDay, phoneNumber, email, code}) => {
    ...
    values(?, ?, ?, ?, ?, ?, ?)`
    await db.query(sql, [username, password, name, birthDay, phoneNumber, email, 'U']);
...
const changPassword = async ({oldPassword, newPassword, reNewPassword}, {username}) => {
